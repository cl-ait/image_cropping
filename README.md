...
このツールは、OpenAI の GPT-4.1モデルを活用して、指定された画像内の特定領域を自動でクロップ（切り抜き）する Python プログラムです。指示文（プロンプト）を入力するだけで、対象物の座標を取得し、クロップ画像を生成・表示します。

---

## 概要

GPT-4.1 を使用して、画像から指定された領域を自動的に検出・クロップします。「にんじんを切ります」「肉を炒めます」といった料理の指示文を入力するだけで、関連する部分を自動的に識別し、16:9のアスペクト比で切り抜きます。
...
```
ValueError: 環境変数 OPENAI_API_KEY が設定されていません。
```
...
日本語が文字化けする場合は、システムに日本語フォントがインストールされているか確認してください。

### JSONパースエラー

GPT-4の応答形式が期待と異なる場合、プログラムが自動修正を試みます。それでも失敗する場合は、`--debug`オプションで詳細を確認してください。
