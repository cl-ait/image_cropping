...
def crop_image_with_gpt(image_path, instruction):
    ...
    - 「ボウルに混ぜます」→ 材料が入ったボウルを探す
    
    指示に従う際のガイドライン：
    1. 食材が画像の 中央付近に来るような範囲を優先してクロップしてください。
    2. 調理器具は、現在使用されている・もしくは使用直前であるもののみを含めてください。
    3. 関連する食材と関係のない調理器具は可能な限り含めないでください。
    4. 明確な調理シーンが存在しない場合は、レシピ指示に最も関係する食材を中央に配置した状態でクロップ範囲を決定してください。
    5. どうしても複数の候補がある場合は、最も関連性が高く、かつ視覚的に分かりやすい部分を優先してください。
    ...
    user_prompt =f"この料理画像から「{instruction}」に関連する部分をクロッピングするための座標を教えてください。レシピ指示に関係する食材を画像の中央付近に含めるようにクロップ範囲を選んでください。"
